...
export default function CareerAddPage() {
  ...
  const { register, handleSubmit, setValue, watch, formState: { errors, isSubmitting } } = useForm<FormData>({
    resolver: zodResolver(schema), defaultValues: { is_published: false },
  });
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      await CareerService.create({ ...data, logo_url, detail_points: [] });
      toast.success("Career created"); router.push("/dashboard/careers");
    } catch (e: unknown) { toast.error("Failed", { description: e instanceof Error ? e.message : undefined }); }
  };

  return (
    <>
      <PageHeader title="Add Career" breadcrumbs={[{ label: "Dashboard", href: "/dashboard" }, { label: "Careers", href: "/dashboard/careers" }, { label: "Add" }]} />
        ...
        <CardContent className="p-6">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2"><Label>Role (ID)</Label><Input {...register("role_id")} />{errors.role_id && <p className="text-xs text-red-500">{errors.role_id.message}</p>}</div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2"><Label>Type (e.g., Contract, Full-time)</Label><Input {...register("type")} /></div>
              <div className="space-y-2"><Label>Model (e.g., Remote, Hybrid)</Label><Input {...register("model")} /></div>
            </div>
            <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2"><Label>Company Logo</Label><ImageUpload accept="image" onChange={(f) => setLogoFile(f)} /></div>
            ...
            <div className="flex justify-end gap-3">
              ...
              <Button type="submit" disabled={isSubmitting} className="bg-neutral-900 text-white dark:bg-white dark:text-neutral-900">{isSubmitting && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Create</Button>
            </div>
          </form>
        </CardContent>
      </Card>

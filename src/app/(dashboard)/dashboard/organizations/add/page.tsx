...
export default function OrganizationAddPage() {
  ...
  const onSubmit = async (data: FormData) => { try { let logo_url: string | null = null; if (logoFile) { const r = await StorageService.uploadImage(STORAGE_PATHS.EXPERIENCES, logoFile); logo_url = r.publicUrl; } await OrganizationService.create({ ...data, logo_url, detail_points: [] }); toast.success("Created"); router.push("/dashboard/organizations"); } catch (e: unknown) { toast.error("Failed", { description: e instanceof Error ? e.message : undefined }); } };
  return (
    <><PageHeader title="Add Organization" breadcrumbs={[{ label: "Dashboard", href: "/dashboard" }, { label: "Organizations", href: "/dashboard/organizations" }, { label: "Add" }]} />
    <Card className="max-w-2xl border-neutral-200/60 bg-white/80 backdrop-blur-sm dark:border-white/10 dark:bg-neutral-900/80"><CardContent className="p-6">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="space-y-2"><Label>Logo</Label><ImageUpload accept="image" onChange={(f) => setLogoFile(f)} /></div>
